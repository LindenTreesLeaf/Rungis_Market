...
<div class="container mx-auto px-4 py-6">
    <h1 class="text-center text-purple-600 text-3xl font-bold mb-6">Paiement de la Commande #{{ $order->id }}</h1>
    ...
    <div class="bg-black text-white p-6 rounded-lg shadow-md">
        <h2 class="text-center text-purple-500 text-xl font-semibold mb-4">Détails de paiement</h2>

        <form action="{{ route('payment.process', $order->id) }}" method="POST">
            @csrf

            <div class="mb-4">
                <label for="card_number" class="form-label text-purple-500">Numéro de carte</label>
                <input type="text" name="card_number" id="card_number" class="form-control border-gray-300 rounded-md w-full p-3 mt-1" required>
            </div>

            <div class="mb-4">
                <label for="expiry_date" class="form-label text-purple-500">Date d'expiration</label>
                <input type="text" name="expiry_date" id="expiry_date" class="form-control border-gray-300 rounded-md w-full p-3 mt-1" required>
            </div>

            <div class="mb-4">
                <label for="cvv" class="form-label text-purple-500">CVV</label>
                <input type="text" name="cvv" id="cvv" class="form-control border-gray-300 rounded-md w-full p-3 mt-1" required>
            </div>
